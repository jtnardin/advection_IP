function [A,Abd] = aMatrixupwind(xn,method)
    ...
    elseif strcmp(method,'upwindfl')

        A = @(se,sw,ve,vw,ind,dn,l) sparse([ind ind ind],[ind-dn ind ind+dn],...
            [l*vw.*(1-sw/2); 1-(l*ve.*(1-1*se/2)-l*vw.*sw/2); -(l*ve.*se/2)],xn,xn);
        
        Abd = @(se,sw,ve,vw,l) sparse([1 1 xn xn],[1 2 xn-1 xn],...
            [1+l*ve.*(1-1*se/2); l*ve.*se/2; -l*vw.*(1-sw/2); 1+(-l*vw.*sw/2)],xn,xn);
    ...
    elseif strcmp(method,'upwindsens')
        ...
        A = @(ve,vw,vs1e,vs1w,vs2e,vs2w,ind,dn,l) ...
            ...
            + sparse([ind+xn ind+xn],[ind ind-dn],[-l*vs1e l*vs1w],3*xn,3*xn) ...
            + sparse([ind+2*xn ind+2*xn],[ind ind-dn],[-l*vs2e l*vs2w],3*xn,3*xn) ...
    ...
    elseif strcmp(method,'laxfriedsens')
        ...
        A = @(ve,vw,vs1e,vs1w,vs2e,vs2w,ind,dn,l) ...
            ...
            + sparse([ind+xn ind+xn],[ind-dn ind+dn],[l*vs1w 1-l*vs1e]/2,3*xn,3*xn)...
            + sparse([ind+2*xn ind+2*xn],[ind-dn ind+dn],[l*vs2w l*vs2e]/2,3*xn,3*xn)...
    ...
    elseif strcmp(method,'laxwendsens')
        ...
        A = @(ve,vw,vp,vs1e,vs1w,vs1p,vs2e,vs2w,vs2p,ind,dn,l) ... 
            ...
            + sparse(repmat(ind+xn,1,3),[ind-dn ind ind+dn],[.5*(l*vs1w+l^2*vs1p.*vs1w) -(l^2)/2*(vs1p.*vs1e + vs1p.^2) ...
              0.5*(-l*vs1e+l^2*vs1e.^2)],3*xn,3*xn)...
            + sparse(repmat(ind+2*xn,1,3),[ind-dn ind ind+dn],[.5*(l*vs2w+l^2*vs2p.*vs2w) -(l^2)/2*(vs2p.*vs2e + vs2p.^2) ...
    ...
    elseif strcmp(method,'beamwarmsens')
        ...
        A = @(ve,vw,vp,vs1e,vs1w,vs1p,vs2e,vs2w,vs2p,ind,dn,l) ...
             ...
             +sparse(repmat(ind+xn,1,3),[ind-2*dn ind-dn ind],[-l/2*vs1w+.5*l^2*vs1w.*vs1p...
               2*l*vs1p-1/2*l^2*vs1p.*vs1e-1/2*l^2*vs1p.^2 -3/2*l*vs1e+1/2*l^2*vs1e.^2],3*xn,3*xn)...
             +sparse(repmat(ind+2*xn,1,3),[ind-2*dn ind-dn ind],[-l/2*vs2w+.5*l^2*vs2w.*vs2p...
               2*l*vs2p-1/2*l^2*vs2p.*vs2e-1/2*l^2*vs2p.^2 -3/2*l*vs2e+1/2*l^2*vs2e.^2],3*xn,3*xn)...
