...
for j = [15 19] %[8 10]
    ...
    for k = 2
        ...
        for num_meth = 3
            ...
            figure('units','normalized','outerposition',[0 0 1 1])
                ...
                if num_meth == 3
                    if j == 15
                        p=polyfit(log(1./xnsize(1:6))',log(J_final(1:6)),1);
                        pb=polyfit(log(1./xnsize(1:6))',log(B_J(1:6)),1);
                        pc=polyfit(log(1./xnsize(1:6))',log(C_J(1:6)),1);
                        pd=polyfit(log(1./xnsize(1:6))',log(F_J(1:6)),1);
                    elseif j == 19
                        p=polyfit(log(1./xnsize(1:4))',log(J_final(1:4)),1);
                        pb=polyfit(log(1./xnsize(1:5))',log(B_J(1:5)),1);
                        pc=polyfit(log(1./xnsize(1:5))',log(C_J(1:5)),1);
                        pd=polyfit(log(1./xnsize(1:5))',log(F_J(1:5)),1);
                    end
                else
                    p=polyfit(log(1./xnsize(1:4))',log(J_final(1:4)),1);
                    pb=polyfit(log(1./xnsize(1:4))',log(B_J(1:4)),1);
                    pc=polyfit(log(1./xnsize(1:4))',log(C_J(1:4)),1);
                    pd=polyfit(log(1./xnsize(1:4))',log(F_J(1:4)),1);
                end
    %             figure(fig_area)
    %             if sigmaj == 1
    ...
    %             figure(fig_log)
                h = 1./(xnsize-1);
                loglog(h,J_final,'x-','linewidth',4,'markersize',20)
                hold on
                loglog(h,abs(A_J*ones(7,1)),'.-','linewidth',3,'markersize',30)
                loglog(h,abs(B_J),'.-','linewidth',3,'markersize',30)
                loglog(h,abs(F_J),'.-','linewidth',3,'markersize',30)
                loglog(h,abs(C_J),'.-','linewidth',3,'markersize',30)
...
%                         hshiftB
                ...
                text(h(end)-1e-4, J_final(end), 'J', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
                text(h(end)-1e-4, A_J-vshiftA, 'A', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
                text(h(end)-1e-4, B_J(end), 'B', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
                text(h(end)-1e-4, C_J(end)-vshiftD, 'D', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
    ...
    %             text(h(end), abs(E_J(end)), 'E', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',8)
                text(h(end)-1e-4, abs(F_J(end))-vshiftC, 'C', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)


                order_str = sprintf(['$p \\approx$ ' sprintf('%.3f',pn)  '\n$ q \\approx$ ' sprintf('%.3f',p(1)) '\n$B$ order $\\approx$ ' sprintf('%.3f',pb(1)) ...
                    '\n$C$ order $\\approx$ ' sprintf('%.3f',pd(1)) '\n$D$ order $\\approx$ ' sprintf('%.3f',pc(1))]);
                text(10^-2,10^-4,order_str,'fontsize',10,'interpreter','latex')


                h=legend('J','A','B','C','D');
                set(h,'units','normalized','position',[.82,.45,.03,.1])

                  title(['$J$ components,  ' num_meth_short_cell{num_meth} ' Method, $N$ = ' num2str(xndata(xdi)) ', $\eta^2 = $' ...
                ...
                elseif strcmp(IC_str,'_front')
                    if num_meth == 1
                        axis([10^-3.3 10^-1.2 10^-5 10^-.6])
                    elseif num_meth == 3 || num_meth == 4
                        axis([5*10^-4 8e-2 2e-5 4e-1 ])
                    end
                end
    ...
    %             num_meth_short_cell{num_meth},' method'],'units','normalized','edgecolor','none','interpreter','latex')
            ...
            exportfig(gcf,['J_comp' IC_str '_' num2str(num_meth) '_' num2str(j) '_manuscript.eps'],'fontsize',2.5,'color','rgb')
            saveas(gcf,['J_comp' IC_str '_' num2str(num_meth) '_' num2str(j) '_manuscript.fig'])
