...
%             figure(fig_log)
            h = 1./(xnsize-1);
            loglog(h,J_final,'.-','linewidth',4,'markersize',35)
            hold on
            ...
            text(h(end)-1e-4, A_J-.5*10^(-2), 'A', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
            text(h(end)-1e-4, B_J(end), 'B', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
            text(h(end)-1e-4, C_J(end), 'C', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
...
%             text(h(end), abs(E_J(end)), 'E', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',8)
            text(h(end)-1e-4, abs(F_J(end)), 'D', 'HorizontalAlignment','center', 'VerticalAlignment','middle','fontsize',10)
                        
            
            order_str = sprintf(['$J$ order $\\approx$' sprintf('%.3f',p(1)) '\n$B$ order $\\approx$' sprintf('%.3f',pb(1)) ...
                '\n$C$ order $\\approx$' sprintf('%.3f',pc(1)) '\n$D$ order $\\approx$' sprintf('%.3f',pd(1))]);
            ...
            set(h,'units','normalized','position',[.93,.45,.03,.1])
              ...
              title(strcat('$J$ components,  ',num_meth_short_cell{num_meth}, ' Method, $N$ = ',num2str(xndata(xdi)),', $\eta^2 = $',...
